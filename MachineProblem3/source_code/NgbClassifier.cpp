...
NgbClassifier::NgbClassifier(std::vector<SampleD> &t, double a) :
...
unsigned int NgbClassifier::classify(SampleD &s){
...
void NgbClassifier::train(){
	...
	if(alpha == 0.0){
		...
		for(unsigned int i = 0; i < prior.size(); i++){
			prior[i] = (double)(n[i]) / (double)( trainset.size() );
		}
	}else{
		// MAP
